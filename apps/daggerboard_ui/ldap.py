...
# TODO configure the logging section here
logging.basicConfig(
    filename=settings.DAGGERBOARD_LOGPATH,
    filemode="a",
...
class GroupLDAPBackend(LDAPBackend):
    ...
    try:
        ...
        logging.info(
            f"Unable to query LDAP settings from database. Settings are null or incorrect. Defaulting to local authentication. - {e}"
        )
        default_settings = {
            "SERVER_URI": "",
            "BIND_DN": "",
    ...
    def authenticate_ldap_user(self, ldap_user: _LDAPUser, password: str):
        ...
        try:
            ...
            if check_server_uri == "":
                logging.info("ldap not configured")
                return None
            else:
